...
- [x] **State management** (`src/context/AppContext.jsx` + `src/utils/dataManager.js`):
  ...
  - Provide these context actions: `addToCart`, `removeFromCart`, `updateCartItemQuantity`, `clearCart`, `placeOrder`, `toggleFavorite`, `setDeliveryMode`, `updateFilters`, `setSearchQuery`, `clearRecentSearches`, `rateOrder`, `reorder`, `updateAddress`, `updateDefaultPayment`, `setTip`, `applyPromoCode`
